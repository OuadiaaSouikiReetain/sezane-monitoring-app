...
const nav = [
  ...
  { to: '/api',         label: 'API Health Hub',   icon: Globe,           badge: null },
...
export default function Sidebar() {
  return (
    <aside className="w-[220px] min-h-screen bg-surface flex flex-col border-r border-white/[0.05]">
      ...
      <div className="px-5 pt-6 pb-5 border-b border-white/[0.05]">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[13px] font-bold text-white leading-none tracking-tight">SFMC Monitor</p>
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        <p className="text-[10px] font-semibold text-slate-600 uppercase tracking-[0.1em] px-3 mb-3">Navigation</p>
        {nav.map(({ to, label, icon: Icon, badge }) => (
          <NavLink
            ...
            className={({ isActive }) =>
              `w-full group flex items-center gap-3 px-3 py-2.5 rounded-xl text-[13px] font-medium transition-all duration-150
              ${isActive
                ? 'text-primary-light'
          ...
          >
            {({ isActive }) => (
              <>
                <Icon size={15} className={isActive ? 'text-primary-light' : 'text-slate-600 group-hover:text-slate-400'} />
                <span className="flex-1 text-left">{label}</span>
                {badge && (
                  <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-md min-w-[18px] text-center
                    ${isActive ? 'bg-primary/30 text-primary-light' : 'bg-white/[0.07] text-slate-400'}`}>
                    {badge}
                  </span>
                )}
              </>
      ...
      <div className="px-4 pb-5 pt-3 border-t border-white/[0.05]">
        <div className="flex items-center gap-3">
          <div className="relative w-8 h-8 rounded-full flex items-center justify-center text-[11px] font-bold text-white flex-shrink-0"
