...
function LatencyBar({ value, max = 900, warn = 200, danger = 500 }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-[11px] font-mono tabular-nums font-semibold w-12 text-right"
        style={{ color }}>{value}ms</span>
    </div>
  )
}

export default function APIHealthHub() {
        ...
        {apis.map((api) => (
          <div key={api.name}
            className={`card transition-all duration-200 hover:scale-[1.02] hover:shadow-card-hover p-4 rounded-2xl
              ${api.status === 'critical' ? 'border-danger/30 bg-danger/[0.04]' :
                api.status === 'degraded' ? 'border-warning/25' : ''}`}>

            {/* Top */}
            <div className="flex items-start justify-between mb-3">
              <StatusBadge status={api.status} />
              <div className={`w-7 h-7 rounded-lg flex items-center justify-center
                ${api.status === 'healthy' ? 'bg-success/10 text-success' :
                  api.status === 'degraded' ? 'bg-warning/10 text-warning' : 'bg-danger/10 text-danger'}`}>
                <Wifi size={13} />
              </div>
            </div>
      ...
      <div className="card p-5">
        <div className="flex items-center justify-between mb-5">
          ...
          <div className="flex gap-4 text-[11px] text-slate-500">
            <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded bg-primary/60" />Avg</span>
            <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded bg-warning/60" />P95</span>
          </div>
        </div>
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={latencyData} barGap={3} barCategoryGap="30%">
            ...
            <Bar dataKey="latency" radius={[4, 4, 0, 0]} maxBarSize={36}>
              {latencyData.map((e) => <Cell key={e.name} fill={e.latency > 200 ? '#F59E0B' : '#6366F1'} fillOpacity={0.85} />)}
            </Bar>
            <Bar dataKey="p95" radius={[4, 4, 0, 0]} maxBarSize={36}>
              {latencyData.map((e) => <Cell key={e.name} fill={e.p95 > 500 ? '#EF4444' : '#F59E0B'} fillOpacity={0.85} />)}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="card overflow-hidden">
        ...
        <table className="data-table">
          ...
          <tbody>
            {apis.map((api) => (
              <tr key={api.name}
                className={api.status === 'critical' ? 'border-l-danger' : api.status === 'degraded' ? 'border-l-warning' : ''}>
                ...
                <td><span className="font-mono text-[12px] text-slate-400 tabular-nums">{api.uptime}</span></td>
              </tr>
            ))}
          </tbody>
        </table>
