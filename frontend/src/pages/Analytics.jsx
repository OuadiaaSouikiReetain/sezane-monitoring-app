...
import { analyticsKpis, performanceTrend } from '../data/mockData'
...
const LINE_LABELS = { successRate: 'Success Rate', deliveryRate: 'Delivery Rate', sla: 'SLA Performance' }
    ...
    <div className="space-y-5 animate-slide-up">
      ...
      <div className="grid grid-cols-4 gap-4">
        {analyticsKpis.map((kpi) => (
          <div key={kpi.label}
            className="card-hover rounded-2xl p-5 transition-all duration-200">
            <p className="text-[11px] font-semibold text-slate-500 uppercase tracking-[0.07em] mb-3">{kpi.label}</p>
            <p className="text-[30px] font-bold text-white leading-none tracking-tight mb-3">{kpi.value}</p>
            <div className={`flex items-center gap-1.5 text-[11px] font-semibold w-fit px-2 py-0.5 rounded-full
              ${kpi.up ? 'bg-success/12 text-success' : 'bg-danger/12 text-danger'}`}>
      ...
      <div className="card p-5">
        ...
        <ResponsiveContainer width="100%" height={280}>
          <LineChart data={performanceTrend} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
            ...
            <YAxis tick={{ fill: '#475569', fontSize: 11 }} axisLine={false} tickLine={false} domain={[89, 100]} unit="%" width={50} />
            <ReferenceLine y={95} stroke="rgba(239,68,68,0.25)" strokeDasharray="4 4" label={{ value: 'SLA target', fill: '#EF4444', fontSize: 10, position: 'insideBottomRight' }} />
            <Tooltip {...TOOLTIP_STYLE} />
            <Legend formatter={(v) => <span className="text-[11px] text-slate-400">{LINE_LABELS[v] || v}</span>} />
            ...
            <div key={bu} className="card-hover rounded-2xl p-5">
              ...
              <div className="space-y-3">
                {[
                  ...
                  const color = value >= warn ? '#6366F1' : value >= warn - 3 ? '#F59E0B' : '#EF4444'
                  return (
                    <div key={label}>
